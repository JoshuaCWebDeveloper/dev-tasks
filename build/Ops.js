...
var exec = require('child_process').exec,
    ...
    argv = require("yargs").argv; //create operations class
...
function () {
  ...
  _createClass(Ops, [{
    ...
    value: function lint() {
      ...
      lint = cli.executeOnFiles([this.__Config.get("sourceDir")]); //output results

      log.info("\n\n".concat(cli.getFormatter()(lint.results), "\n")); //if there were problems
    ...
    value: function bundle(env, minify) {
      ...
      var env = env || "development",
          ...
          //define filename
      outFn = minify ? "".concat(bName, ".min.js") : "".concat(bName, ".js"),
      ...
      wpConfig = extend(true, {
        ...
        module: {
          rules: [//transform app js files using babel
          {
            test: /\.js$/,
      ...
      }, this.__Config.get("wpExtOptions")); //out operation info


      log.info("Starting WebPack compiler, output to: ".concat(wpConfig.output.path, "/").concat(wpConfig.output.filename)); //create and run webpack compiler (use Q promise)
    ...
    value: function release() {
      ...
      reqs = {
        ...
        modFiles: ["".concat(bundleProps.bundleDir, "/").concat(bundleProps.bundleName, ".js"), "".concat(bundleProps.bundleDir, "/").concat(bundleProps.bundleName, ".min.js")]
      ...
      props.type = argv._.length ? argv._[1] : undefined; //if we didn't receive a valid type

      if (['major', 'minor', 'patch'].indexOf(props.type) < 0) {
        throw new Error("Missing required argument: 'major|minor|patch' (received: '".concat(props.type, "')"));
      ...
      return Q(Git.Repository.open("./")).then(function (repo) {
        ...
        var queries = []; //store repo

        props.git.repo = repo; //look for the required remote

        queries.push(Git.Remote.lookup(repo, reqs.remote)["catch"](function (err) {
          //FAILURE
          //ourput error, we couldn't find the right repo
          throw new Error("Unable to find remote named '".concat(reqs.remote, "'. ").concat(err || ''));
        })); //check our current branch name

        queries.push(repo.getCurrentBranch()); //check status of our repo

        queries.push(repo.getStatus()); //check for existance of changelog-release.txt

        queries.push(Q.nbind(fs.readFile)(props.changelogName, 'utf8')["catch"](function (err) {
          //FAILURE
          throw new Error("Unable to find file ".concat(props.changelogName, ". Create one following changelog specifications. ").concat(err || ''));
        })); //load the index for our repo

        queries.push(repo.refreshIndex()); //load current HEAD

        queries.push(repo.getHeadCommit()); //load the default git config

        queries.push(Git.Config.openDefault()); //lookup the master branch

        queries.push(repo.getBranchCommit("master")); //return the result of all of our queries

        return Q.all(queries);
      }, function (err) {
        //FAILURE
        throw new Error("Not currently in app root directory, please cd. ".concat(err || ''));
        ...
        status = results[2],
            statusFail = false; //get the remote from our first query

        props.git.remote = results[0]; //get the changelog data from our fourth query

        props.changelogContents = results[3]; //get the index from our fifth query

        props.git.index = results[4]; //get the head commit from our sixth query

        props.git.head = results[5]; //get git config from our seventh query

        props.git.config = results[6]; //get master commit from our eighth query

        props.git.master = results[7]; //if our current branch isn't the required branch

        if (curBranch.shorthand() != reqs.branch) {
          //then we aren't on the right branch
          throw new Error("Branch '".concat(reqs.branch, "' not checked out."));
        ...
        if (statusFail) {
          throw new Error("Unrecognized or missing modified files. \nEnsure that all of and only the following files are modified: ".concat(reqs.modFiles.join(', ')));
        } //ensure that the merge base between our branch and master is master


        tests.push(Git.Merge.base(props.git.repo, props.git.head, props.git.master)); //return results of all of our tests

        return Q.all(tests);
      }).then(function (results) {
        //SUCCESS
        //store baseOid from first test
        var baseOid = results[0]; //if the base is NOT master
        ...
        return Q.nbind(exec)("npm --no-git-tag-version version ".concat(props.type));
      }).then(function () {
        //SUCCESS
        //create collection of ops to perform at once
        var ops = []; //add bundle.js, bundle.min.js, and package.json to index

        ops.push(props.git.index.addByPath('package.json'));
        ops.push(props.git.index.addByPath("".concat(bundleProps.bundleDir, "/").concat(bundleProps.bundleName, ".js")));
        ops.push(props.git.index.addByPath("".concat(bundleProps.bundleDir, "/").concat(bundleProps.bundleName, ".min.js"))); //if package-lock.json exists, add it too
        ...
        ops.push(props.git.config.getStringBuf("user.email")); //return result of all of ops

        return Q.all(ops);
      }).then(function (results) {
        //SUCCESS
        //create a collection of tasks to perform at once
        var tasks = []; //get the user's name and email from our fifth and sixth ops

        props.git.username = results[4];
        props.git.useremail = results[5]; //write to the index

        tasks.push(props.git.index.write()); //return result of all tasks
        ...
        //SUCCESS
        //store tree
        props.git.tree = treeOid; //create a signature for the committer

        props.committer.signature = Git.Signature.now(props.committer.name, props.committer.email); //create a signature for the author

        var author = Git.Signature.now(props.git.username, props.git.useremail),
            //create a message for our commit
        message = "Update ".concat(bundleProps.bundleName, ".js, ").concat(bundleProps.bundleName, ".min.js, and package.json version"); //commit our changes to HEAD
        ...
        annotation = "".concat(props.appName, " ").concat(tagName, " ").concat(os.EOL).concat(props.changelogContents),
            //init a collection of jobs to perform at once
        jobs = []; //save tage name


        props.tagName = tagName; //create a new tag for our commit

        jobs.push(props.git.repo.createTag(commitOid, tagName, annotation)); //fast-forward our branch onto master (have already checked to ensure ff merge)

        jobs.push(props.git.repo.mergeBranches('master', reqs.branch, props.committer.signature)); //update the changelog file

        jobs.push(Q.nbind(fs.writeFile)("./changelog-".concat(tagName, ".txt"), annotation));
        jobs.push(Q.nbind(fs.unlink)(props.changelogName)); //return the result of all jobs
        ...
        return Q.nbind(exec)("git push ".concat(reqs.remote, " master:master ").concat(props.tagName));
      }).then(function () {
        log.info("\n\nSuccessfully released ".concat(props.tagName, "!\n"));
      });
    }
  }]);
